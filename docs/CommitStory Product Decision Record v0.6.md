# CommitStory — Product Decision Record
## Core V1 Product Decisions

**Status:** Draft v0.6 (supersedes v0.5 — Scheduling and AI undo/versioning decisions finalized)  
**Purpose:** Canonical record of CommitStory's core V1 product/schema decisions. This document contains the locked V1 baseline, finalized scheduling behavior (Part 20), finalized AI undo/versioning behavior (Part 21), and the remaining V1 schema-design work.
**Date of Creation:** September 5, 2026  
**Product:** CommitStory (working product name; final naming to be decided later)
...
- no public CommitStory social profile/page in V1
...
> “I have authorized CommitStory to access this platform/account.”
...
and explicitly wanted CommitStory to support those.
...
If the current draft differs from the scheduled version, CommitStory shows:
...
The user also added that deleting a published post from the external platform **from inside CommitStory** is a valuable UX feature.
...
**Deleting/removing CommitStory's local data**
...
But CommitStory should also support an explicit **Delete from X / Delete from LinkedIn** operation for published posts.
...
## Q29. Should users choose an existing X post from inside CommitStory for reply/quote?

### Question

Should the user be able to select an existing X post from inside CommitStory when creating:
...
## Q30. Which existing X posts should CommitStory show?

### Question

Should CommitStory show:

- only posts published through CommitStory
- or actual recent X posts, including posts published outside CommitStory?
...
The feature should work with the user's real X history, not just CommitStory-created posts.
...
## Q32. Should CommitStory show how the post will look before publishing?
...
## Q35. Should CommitStory eventually import/understand existing X threads created outside CommitStory?
...
But the user wants the CommitStory work to remain:
...
**Delete external publication, preserve CommitStory work.**

The UX should be explicit:

> **Delete from X**
>
> This permanently removes the post from X. Your CommitStory draft and content will remain available for editing.
...
## Q41. When should CommitStory refresh publication status from X/LinkedIn?
...
The user asked what the practical difference is and suggested that CommitStory could potentially refresh status from the platform when the user interacts with the publication.
...
CommitStory can no longer publish them.
...
The user additionally wants CommitStory to be aware that scheduled posts can no longer publish after disconnection.
...
**Disconnect does not delete the remembered account from CommitStory.**

The account can later be reconnected.

### 3. Delete

Remove the remembered connected-account record from CommitStory.
...
When the user schedules a draft, CommitStory creates an approved scheduled snapshot.
...
CommitStory asks for confirmation before replacing the scheduled snapshot.
...
At scheduling time, CommitStory freezes everything required to reproduce the exact approved publication, without blindly duplicating unrelated draft state.
...
CommitStory V1 stores AI generation history.
...
This keeps AI-generation lifecycle tied to the lifecycle of the underlying CommitStory data.
...
The future `CommitStory-domain-schema-spec.md` will translate these decisions into concrete entities, relationships, constraints, lifecycle rules, indexes, and Drizzle mappings.
...
### Old idea
> Deleting a publication could mean deleting local CommitStory data.
