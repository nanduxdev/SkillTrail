# Engineering Specification — SkillTrail V1
...
**Purpose:** Turn SkillTrail's current product decisions into concrete engineering choices, runtime boundaries, application modules, background-task responsibilities, data-flow rules, and build constraints so agentic development can proceed without repeatedly revisiting architecture or product behavior.  
...
This document is the engineering implementation contract for SkillTrail V1.

It is derived from:

```text
PRD SkillTrail v0.3.md
SkillTrail Product Decision Record v0.6.md (includes Scheduling — Part 20 — and AI Undo/Versioning — Part 21)
```
...
SkillTrail V1 is optimized for:
...
Trigger.dev provides durable background execution without requiring SkillTrail to operate its own worker infrastructure.
...
The SkillTrail domain layer must not depend on Bun-only APIs.
...
`understandContent` follows PRD §7: it extracts a story from raw material and does not require a shipped software artifact. If there is no meaningful insight, it should not force angles or a post.

The rest of SkillTrail must not directly depend on Gemini-specific SDK calls.
...
The final relational schema is intentionally deferred to `SkillTrail-domain-schema-spec.md`.
...
- raw notes
- what they built
- notes from a problem they worked through (approaches, mistakes, what clicked)
- lessons
- uploaded context
...
After connecting X, SkillTrail can work with the user's actual recent X posts, including posts created outside SkillTrail.
...
Importing/understanding existing X threads created outside SkillTrail is P1.
...
Deleting an externally published post is a separate action from deleting local SkillTrail data.
...
Keep Better Auth's authentication records separate from SkillTrail's publishing authorization records.
...
No public SkillTrail profile/social-network layer is required in V1.
...
- public SkillTrail social profiles
...
```text
SkillTrail-domain-schema-spec.md
```
...
When an AI coding agent implements SkillTrail:
...
SkillTrail V1 is intentionally a **single Next.js application backed by Neon, R2, Better Auth, hosted AI APIs, and Trigger.dev**.
...
This gives SkillTrail enough structure to demonstrate production-grade engineering while keeping the system small enough for a solo developer to build, operate, and iterate within the initial budget.
