...
**User goal:** *"I did something interesting. Help me turn it into a post."* This is the most important workflow in V1. Interesting includes shipping something, working through a hard problem, or learning something worth sharing — same dump box, no extra form.

**Input** — any combination of, none required exclusively:

- **Text:** what they built, worked on, tried, what went wrong, what they learned, raw thoughts — including notes from a problem they worked through (failed approaches, what finally clicked).
...
- **Solution** — what changed: a built artifact, a different approach, or a change in understanding? Not every session produces shipped code.
- **Challenges** — what went wrong, required effort, or was a mistaken approach?
- **Learning** — what did the user learn?
- **Interesting details** — what would make this useful/interesting to another developer? Infer extra specifics only when they are in the material (for example a technique, a complexity observation, a breakthrough, or how their approach improved). Do not invent them or require a dedicated form.

Then decide whether there is a story:

```text
Activity
   ↓
Meaningful insight / story?
   ↓
Yes → Generate possible content angles
No  → Don't force a post
```

Thin activity-count material (for example "solved five problems today") is not enough. Prefer the actual insight — a failed brute-force attempt, a pattern that finally clicked — over summarizing volume.
...
- **Learning:** "One thing I learned while working on X..."
- **Unexpected challenge:** "I thought X would be simple. It wasn't."
- **Technical insight:** "Here's what surprised me about X..." / "The breakthrough was recognizing I didn't need brute force..."
...
```text
Finishes today's work or a problem they worked through → Opens Create → Dumps everything into one box → Optional screenshot
→ AI identifies whether there is a story → Angles (only if there is one) → LinkedIn + X → Edit → Preview & verify → Publish / Schedule
```
The system doesn't need to know whether this is Day 3, Day 37, Day 91, or simply Tuesday. Same Create box; no dedicated problem-tracking workflow.
...
```text
WORK / LEARN → CAPTURE → CREATE → REVIEW → PREVIEW → SHARE → RETURN
```
