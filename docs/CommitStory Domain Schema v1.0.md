# CommitStory — Domain & Schema Specification v1.0

**Status:** LOCKED — synchronized with the provided `CommitStory-Schema-Spec-v1.0.md`  
**Purpose:** Canonical domain/database contract for CommitStory V1. This document records the finalized entities, ownership model, relationships, lifecycle rules, constraints, indexes, deletion behavior, transaction boundaries, and Drizzle/PostgreSQL decisions that the implementation must follow.  
...
```text
CommitStory Product Decision Record v0.6
CommitStory Engineering Spec v0.3
CommitStory-Schema-Spec-v1.0.md
```
...
23 CommitStory-owned tables
27 total tables in the inventory
```

This count is canonical. Do not use an older “25 CommitStory-owned tables” figure.
...
The username plugin provides `username` and `displayUsername` on Better Auth's `user` table. CommitStory must not duplicate them onto `application_profile`. fileciteturn29file0L42-L44

Every CommitStory-owned user reference is a `text` FK to Better Auth `user.id`.
...
It stores only CommitStory-specific personalization.
...
- An X or LinkedIn account can belong to only one CommitStory user.
- The same CommitStory user may have multiple accounts on the same platform, because `user_id` is **not** part of this unique constraint.
...
A single CommitStory user may have multiple active X/LinkedIn accounts simultaneously.
...
Disconnect is a user action. `expired` and `revoked` are operational statuses on the same enum. Deleting removes the remembered CommitStory connection.
...
Reconnect (user action) returns the row to `connected`. Explicit disconnect is `disconnected`. Deleting the remembered row removes it from CommitStory.
...
A published external post can be deleted from CommitStory.
...
**Canonical:** 23 CommitStory-owned tables + 4 Better Auth tables (27 in the inventory).
